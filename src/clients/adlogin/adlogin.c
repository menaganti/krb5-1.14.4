...
static char *
parse_adlogin_options(argc, argv, opts)
    int argc;
    char **argv;
    struct k_opts* opts;
{
    ...
    while ((i = GETOPT(argc, argv,
        ...
        case 'd':
            if (domainName != "")
            {
                fprintf(stderr, _("Only one -d option allowed.\n"));
                errflg++;
            } else {
                domainName = argv[2];
            }
        ...
        case 'n':
            if (userName != "") {
                fprintf(stderr, _("Only one -n option allowed\n"));
                errflg++;
            } else userName = argv[4];
		printf("User name: %s\n",userName);
            break;
        default:
            errflg++;
    ...
    printf("No of Args opts vs argc: %d vs %d\n", optind, argc);
    strcat(userName, "@");
    strcat(opts->principal_name, userName);
    strcat(opts->principal_name, domainName);
    /*opts->principal_name = (optind == argc-1) ? argv[optind] : 0;*/
    command[2] = opts->principal_name;
    return parse_adlogin_options(2, command, opts->principal_name);
}
